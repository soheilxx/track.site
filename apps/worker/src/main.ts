...
/** Health for orchestrators: process alive, loops running, queue reachable, scheduled jobs with their last run. Never exposed publicly. */
  ...
  try {
    ...
    res.writeHead(200, { "content-type": "application/json" }).end(
      JSON.stringify({
        ...
        queue: { driver: ctx.queue.driver, ready: stats?.ready ?? null, dlq: stats?.deadLetters ?? null },
        jobs: jobs.status(),
