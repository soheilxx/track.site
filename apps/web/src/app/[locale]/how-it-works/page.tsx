...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return pageMetadata({ locale, path: "/how-it-works", title: seoTitle(c.title), description: seoDescription(c.intro) });
}
...
export default async function HowItWorksPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <JsonLd data={[breadcrumbJsonLd([{ name: BRAND_NAME, path: "/" }, { name: l.eyebrow, path: "/how-it-works" }], locale), faqJsonLd(c.faq), { "@context": "https://schema.org", "@type": "HowTo", name: c.title, step: c.steps.map((s, i) => ({ "@type": "HowToStep", position: i + 1, name: s.title, text: s.text })) }]} />
