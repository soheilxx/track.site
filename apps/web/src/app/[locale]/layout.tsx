 ...
 *
 * The client provider carries no messages: every public page translates on the server (typed copy
 * modules, `getTranslations`), the client islands only read the locale. Namespaces a client subtree
 * does need are added by its route group (`(auth)/layout.tsx` → `auth`), so the message catalog
 * (200 KB JSON per locale) never travels in the RSC payload of a marketing page.
 */
