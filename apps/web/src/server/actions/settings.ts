...
export async function updateOrganizationAction(
  ...
  const api = auth().api as unknown as {
    updateOrganization: (args: {
      body: { data: { name: string }; organizationId: string };
      headers: Headers;
    }) => Promise<unknown>;
  };
  try {
    await api.updateOrganization({
      body: { data: { name: parsed.data.name }, organizationId: ctx.organization.id },
...
export async function updateOrgSettingsAction(
  ...
  const parsed = settingsSchema.safeParse({
    ...
    dataRegion: formData.get("dataRegion") ?? "eu",
    aiEnabled: formData.get("aiEnabled") === "on",
    benchmarkOptIn: formData.get("benchmarkOptIn") === "on",
  ...
  await withOrg(ctx, async (tx) => {
    await tx
      .insert(orgSettings)
      .values({ organizationId: ctx.organization.id, ...parsed.data })
      .onConflictDoUpdate({ target: orgSettings.organizationId, set: parsed.data });
    await recordAudit(tx, {
      organizationId: ctx.organization.id,
      actor: ctx.tenant.actor,
      action: parsed.data.killSwitch ? "org.kill_switch_on" : "org.settings",
      targetType: "organization",
      targetId: ctx.organization.id,
      diff: parsed.data,
      requestId: ctx.tenant.requestId,
...
export async function updateSiteAction(
  ...
  const ok = await withOrg(ctx, async (tx) => {
    ...
    await tx
      .update(sites)
      .set({ name: parsed.data.name, killSwitch: parsed.data.killSwitch })
      .where(eq(sites.id, site.id));
    await recordAudit(tx, {
      organizationId: ctx.organization.id,
      actor: ctx.tenant.actor,
      action: parsed.data.killSwitch ? "site.kill_switch_on" : "site.update",
      targetType: "site",
      targetId: site.id,
      diff: parsed.data,
      requestId: ctx.tenant.requestId,
...
export async function createSourceKeyAction(
  _prev: KeyState,
  formData: FormData,
): Promise<KeyState> {
  const ctx = await requireOrgContext("sites.update");
  const parsed = z
    .object({ siteId: z.string().uuid(), name: z.string().trim().min(1).max(60) })
    .safeParse({ siteId: formData.get("siteId"), name: formData.get("name") });
  if (!parsed.success) return { ok: false, error: "generic" };
  ...
  const result = await withOrg(ctx, async (tx) => {
    ...
    await recordAudit(tx, {
      organizationId: ctx.organization.id,
      actor: ctx.tenant.actor,
      action: "source_key.create",
      targetType: "source_key",
      targetId: row!.id,
      diff: { site: site.id, name: parsed.data.name },
      requestId: ctx.tenant.requestId,
    });
    return row!.id;
  });
...
export async function revokeSourceKeyAction(
  _prev: ActionState,
  formData: FormData,
): Promise<ActionState> {
  const ctx = await requireOrgContext("sites.update");
  ...
  await withOrg(ctx, async (tx) => {
    await tx
      ...
      .where(and(eq(sourceKeys.id, keyId), eq(sourceKeys.organizationId, ctx.organization.id)));
    await recordAudit(tx, {
      organizationId: ctx.organization.id,
      actor: ctx.tenant.actor,
      action: "source_key.revoke",
      targetType: "source_key",
      targetId: keyId,
      requestId: ctx.tenant.requestId,
    });
...
export async function updateLocaleAction(locale: "en" | "de"): Promise<void> {
  ...
  await db()
    ...
    .onConflictDoUpdate({ target: orgSettings.organizationId, set: { locale } });
  revalidatePath("/app");
}

/**
 * Per-user Track AI motion preference (supplement §9 "Kontrolle, Barrierefreiheit"): system default,
 * full, reduced or off. Stored in workspace_preferences.ai_motion for the signed-in user inside the
 * active organization; any member may set their own preference. Audited without the previous value
 * (an accessibility preference, no tenant data).
 */
export async function updateAiMotionAction(
  _prev: ActionState,
  formData: FormData,
): Promise<ActionState> {
  ...
  await writeAiMotion(ctx, value);
  await withOrg(ctx, (tx) =>
    recordAudit(tx, {
      organizationId: ctx.organization.id,
      actor: ctx.tenant.actor,
      action: "workspace.ai_motion",
      targetType: "workspace_preference",
