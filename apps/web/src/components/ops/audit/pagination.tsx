...
/** Page links of the explorer; `query` carries every other filter (built on the client side of the boundary). */
export function AuditPagination({ page, pageCount, query }: { page: number; pageCount: number; query: string }) {
  const t = useTranslations("opsAudit.pagination");
  const hrefFor = (p: number) => {
    const params = new URLSearchParams(query.replace(/^\?/, ""));
    if (p > 1) params.set("page", String(p));
    else params.delete("page");
    const s = params.toString();
    return `/ops/audit${s ? `?${s}` : ""}`;
  };
