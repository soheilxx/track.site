...
/** Section of the growth page: one h2, a one-line intro, an optional aside (figures) on the right, then the content. */
...
/** Small labelled figure used in section asides ("Last 7 days · 3"). */
...
export function ShareBar({ share, className }: { share: number | null; className?: string }) {
  ...
  return (
    <span aria-hidden="true" className={cn("block h-2 w-full max-w-40 overflow-hidden rounded-[var(--radius-chip)] bg-surface-2", className)}>
      <span className="block h-full rounded-[var(--radius-chip)] bg-primary" style={{ width: `${width}%` }} />
    </span>
  );
}
  ...
  return (
    <details className="group rounded-[var(--radius-control)] border border-line bg-surface">
      <summary className="flex min-h-11 cursor-pointer list-none items-center gap-2 px-3 text-sm font-medium text-ink-2 hover:text-ink focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary [&::-webkit-details-marker]:hidden">
        <span aria-hidden="true" className="inline-block transition-transform duration-[var(--motion-fast)] group-open:rotate-90">
          ▸
        </span>
        {summary}
      </summary>
