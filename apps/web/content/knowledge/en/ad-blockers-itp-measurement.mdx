...
level: "intermediate"
takeaways:
  - "Content blockers, browser tracking prevention and consent refusal are three different losses with different fixes — and consent refusal is not a loss to fix."
  - "Measure your own gap: compare the collector's accepted page views with the vendor tag's count, read the refusal share on the consent page and the returning-visitor ratio per browser."
  - "A first-party, server-side setup recovers consented events that list-based blockers drop, delivers confirmed purchases and leads from the server and retries vendor outages."
  - "It does not lift Safari's cap on script-written storage, restore refused consent, inject blocked vendor pixels or replace identifiers with fingerprinting."
