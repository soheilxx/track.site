...
description: "Why a theme script cannot see Shopify's checkout, how the Track web pixel extension and signed orders/paid webhooks work together, what the collector verifies, and how the server purchase inherits the customer's consent and click ids."
...
level: "intermediate"
takeaways:
  - "Shopify hosts the checkout, so a theme script cannot see the purchase; the web pixel extension covers the browser side and reads consent from the Customer Privacy API."
  - "orders/paid and refunds/create webhooks are verified by HMAC-SHA256 signature and shop domain and become source-verified purchase and refund events with deterministic ids."
  - "The router pairs the server purchase with the browser purchase by order id so it inherits the consent record, anonymous id, click ids and hashed identifiers."
  - "Without a browser purchase the server record stays operational and never reaches advertising platforms — a paying customer is not assumed to have consented."
