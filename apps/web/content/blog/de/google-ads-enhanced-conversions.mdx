...
- Klicks, die nur wenige Stunden alt sind, sind eventuell noch nicht zuordenbar (`TOO_RECENT_CLICK`). Später erneut zu versuchen ist korrekt; track.site behandelt diese Fälle als wiederholbar.
...
1. Google-Konto per OAuth verbinden und validieren: track.site sendet einen Validate-only-Upload und meldet, ob Developer-Token und Kunden-ID akzeptiert werden.
