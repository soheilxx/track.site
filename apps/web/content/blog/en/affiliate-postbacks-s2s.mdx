...
These presets are declarative templates in track.site; the wizard shows the rendered URL or body with placeholders before you activate anything, and the parameter names above come from each network's current advertiser documentation, linked in the destination's help text.

## Inbound: when the network calls you

Some platforms report the sale to you instead (Digistore24's IPN, or a network's postback to your own endpoint). track.site exposes an inbound URL per site and preset. It verifies the request — Digistore24's `sha_sign`, a shared secret token for generic presets — and turns it into a verified `purchase` or `refund` event that flows through the same router. Unverified requests are rejected and counted in the destination's health.
...
Most networks provide a test mode flag (Awin `testmode=1`, Impact test accounts, TUNE test offers). In track.site's test mode the preset's test parameter is set automatically where one exists; where none exists, the wizard sends to the network and shows the response, and you validate the transaction in the network UI before switching the destination live.
