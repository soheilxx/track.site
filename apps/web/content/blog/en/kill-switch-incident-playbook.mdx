...
Steps: rotate the credential in the vault (the old one is invalidated on the vendor side by you, then replaced in track.site); pausing the destination in between avoids a burst of auth failures. The audit log shows who read or rotated the credential; the value itself was never logged.
