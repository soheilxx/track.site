...
Most implementations go wrong by wiring a vendor list to the flags. A cleaner model — the one track.site uses — derives the flags from consent purposes:
...
**Advanced** mode: Google tags load immediately and send cookieless pings while consent is denied, so Google can model conversions later. Whether those pings are acceptable is a legal assessment of your situation, not a technical setting. track.site therefore keeps basic mode as the default and only enables advanced mode when a legal review note is stored with the consent policy — the assistant cannot switch it silently.
