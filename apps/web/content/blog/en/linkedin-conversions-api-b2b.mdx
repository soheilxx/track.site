...
2. An **OAuth 2.0 token** with the `r_ads` and `rw_conversions` scopes, issued to a user with access to the ad account. track.site connects the account through LinkedIn OAuth and refreshes the token automatically.
...
Versioning is monthly. The `LinkedIn-Version` header names a version that stays valid for about a year; track.site pins it in its version registry and warns before the sunset date.
