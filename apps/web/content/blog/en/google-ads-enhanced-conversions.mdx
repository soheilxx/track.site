...
- Clicks younger than a few hours may not be matchable yet (`TOO_RECENT_CLICK`). Retrying later is correct behaviour; track.site treats these as retryable.
...
1. Connect the Google account through OAuth and validate: track.site sends a validate-only upload and reports whether the developer token and customer id are accepted.
