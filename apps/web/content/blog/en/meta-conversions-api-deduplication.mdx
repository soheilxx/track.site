...
`POST https://graph.facebook.com/{version}/{pixel_id}/events` with a system-user access token and a JSON body containing a `data` array of events. track.site pins the Graph API version centrally (v25.0 at the time of writing) and records when the endpoint was last verified against Meta's documentation.
...
track.site does this by construction: the SDK generates a source event id, mirrors the pixel call with that id, and the worker sends the same id in `event_id`.
...
Not hashed: `client_ip_address`, `client_user_agent`, `fbc`, `fbp`. The `fbc` value is built from the `fbclid` URL parameter as `fb.1.{timestamp}.{fbclid}`; `fbp` is the `_fbp` cookie set by the pixel. Both are captured by the track.site SDK only after marketing consent and forwarded only to Meta.
...
2. Store it in the destination's settings; track.site attaches it only while the destination is in test mode.
...
- **4 / 17 / 32 / 613** — rate limits: track.site backs off with jitter and retries
