...
description: "How track.site's setup assistant is built so that it can plan and configure but never execute code, invent data or bypass consent — the tool contract, server-side validation, approval tokens, the audit trail and what the model never sees."
