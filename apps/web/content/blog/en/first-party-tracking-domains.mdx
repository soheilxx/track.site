...
- Add the domain in the site settings. track.site issues a **verification token**; prove control with a DNS TXT record, a file on the domain or a meta tag. The domain becomes active only after verification succeeds, so nobody can point a domain they do not control at your site.
...
track.site writes three things in the browser, each only after the matching consent:
