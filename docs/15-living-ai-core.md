...
Acceptance evidence (supplement §9 "Verbindliche Abnahmekriterien"), status after the follow-up run of 2026-09-05 (`docs/16-release-report.md` §6.6, §9; all on the production build `JkuZkqiqEn0HgN4FIDCyT` served with the stored owner session, chromium headless shell with `--use-angle=d3d11` unless stated):

| Criterion | Status | Evidence |
| --- | --- | --- |
| Reduced motion / setting `off` / hidden tab: no continuous animation frames | **pass** — 0 `requestAnimationFrame` callbacks in 10 s windows under `prefers-reduced-motion: reduce`, after the header toggle to `off` and after a reload with the persisted `off`; `data-tier="static"` in the SSR HTML, no canvas, no running animation inside `.lac`; the WebGL2 context was released on the toggle (`webglcontextlost` 1). Hidden tab: unit test `living-ai-core.dom.test.tsx` (visibility pause) — not measured in a browser | `docs/qa/2026-09-05/followup/living-core/motion-off.json`, `motion.log`, `summary.md` §(d) |
| Lighthouse median (3 mobile runs) at most 3 points below the static chat version | **FAIL — 6 points**: panel docked (mobile preset, screen widened to 1280 × 800 @ DPR 2 so the core mounts on the `webgl` tier, verified in `lighthouse/tier-checks.json`): `off` 75 / 79 / 77 → median 77; `system` 70 / 71 / 71 → 71; `full` 76 / 71 / 70 → 71; TBT +209 / +198 ms, LCP +81 / +83 ms, CLS +0.001; control on the default phone emulation (panel closed, core not mounted) 83 vs 83. `full` / phone group missing (runner stopped). Defect D17 in docs/16 §9 | `docs/qa/2026-09-05/followup/living-core/lighthouse/summary.md`, `summary.json`, `app--{off,system,full}--docked--run{1,2,3}.report.json`, `app--{off,system}--phone--run{1,2,3}.report.json`, `run.log`; runner `apps/web/scripts/qa/living-core-lighthouse.mjs` |
| Idle animation causes no attributable long tasks > 50 ms | **not run** (the E1 session was stopped before the check started); harness `apps/web/scripts/qa/living-core-budget.mjs --mode longtasks` ready and smoke-tested in `tier` mode | `docs/qa/2026-09-05/followup/living-core/summary.md` §(a), `tier-smoke-1440.json` |
| ≥ 30-minute run with state changes: no growth of memory, listeners, canvases, WebGL resources | **not run** (same session); `--mode soak` implements the 20 s state cycle with the provider stubbed at `fetch` and CDP samples every 60 s, untested end-to-end | `summary.md` §(b) |
