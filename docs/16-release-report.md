...
| 7 SEO gate (`pnpm seo:check`) | **exit 1** — 202 problems; 198 are stale checker rules (SVG `<title>` counted, BlogPosting-only rule), 4 are real description overruns | §7, defects D4, D5 |
...
| Social cards of Tracking Knowledge pages | **fail** — `og:image` of the hub and all 180 articles answers 404 | defect D1 |
...
The programme is complete as far as the repository can make it; the release is **not** "done" in the sense of §11 until defect D1 (social cards) is fixed and the gate-blocking tooling defects D5–D7 are resolved, and it ships with a documented performance gap (D9).
...
| D7 | `pnpm test:integration` deadlocks when db, worker and web run their `global-setup.ts` migration of `tracksite_test` concurrently | major (gate; CI runs the same command) | platform / db | `reports/test-integration-run1-parallel.txt` | serialize the global setup (pg advisory lock or one shared migration task) |
| D8 | `docs/redirects-blog-to-tracking-knowledge.md` still lists 2 active locales (generated 2026-09-03); the six-locale matrix lives in the evidence pack | minor (docs) | web | §4 | run `node apps/web/scripts/redirect-matrix.mjs` (it reads `ACTIVE_LOCALES`) |
...
| D13 | The F1 fixes and the QA tooling are **uncommitted** (35 modified files, 5 untracked paths) — the evidence in `recheck/` and `reports/` was produced from that working tree | blocking for release | integrator | `changed-files.md` §3 | commit on the feature branch after D6 (lint) is resolved |
