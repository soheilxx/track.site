...
- Two-factor reset (docs/17 §12): an administrator can remove a person's TOTP secret and backup codes, switch two-factor off and revoke every session — never their own account; platform admins for other platform users (reason) and, as a support tool, for customer accounts (reason, ticket reference, the ticket's organisation on the audit row); organisation owners/admins for members of their own organisation (`members.security`, an owner only by an owner, never a member with a platform role, refused under break-glass). Every reset is confirmed, audited without secrets and e-mailed to the affected person in their language with the support contact.
