...
| `alerts` | 60 s | evaluates the enabled `alert_rules` (event drops and conversion anomalies against the same window 1–4 weeks earlier from `event_aggregates`, consent-blocked share, error rate/status and queue age from `destination_health_snapshots`, credential and OAuth expiry), writes `alert_events` with per-subject cooldown and auto-resolution, delivers e-mail (`SMTP_URL` → `RESEND_API_KEY` → local file outbox outside production), HMAC-signed webhooks (`X-Track-Signature`) and Slack blocks with 10 s timeouts and records the outcome per channel | Alerts & Incident Mode (`/app/settings/alerts`) |

Until a job has run once, the pages it feeds show "not measured" rather than a value. `scheduled-publish` needs `CONFIG_SIGNING_PRIVATE_KEY` and `CONFIG_SIGNING_KEY_ID` on the worker (same values as the web app); without them due drafts are refused with `signing_key_missing` and shown as such in the release center. `alerts` decrypts webhook/Slack URLs with the worker vault (`MASTER_KEY` / KMS, same key as the web app) and reads `SMTP_URL`, `RESEND_API_KEY`, `MAIL_FROM` and `HOST_APP` (deep links) from the process environment; a missing transport or vault is recorded on the event (`delivery[channel].error`), never thrown.
